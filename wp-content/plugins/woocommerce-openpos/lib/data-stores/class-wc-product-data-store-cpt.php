...
class OP_WC_Product_Data_Store_CPT extends WC_Product_Data_Store_CPT {
	...
	public function search_products( $term, $type = '', $include_variations = false, $all_statuses = false, $limit = null, $include = null, $exclude = null ) {
		...
		$search_content = apply_filters(
			'op_woocommerce_allow_search_product_content',
			true
		);
		...
		foreach ( $term_groups as $term_group ) {
			...
			foreach ( $search_terms as $search_term ) {
				...
				if($search_content)
				{
					...
					$term_group_query .= $wpdb->prepare( " {$searchand} ( ( posts.post_title LIKE %s) OR ( wc_product_meta_lookup.sku LIKE %s ) )", $like, $like ); // @codingStandardsIgnoreLine.
